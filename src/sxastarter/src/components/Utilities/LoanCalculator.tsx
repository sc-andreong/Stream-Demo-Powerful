...
export const Default = (props: LoanCalculatorProps): JSX.Element => {
  ...
  const [loanAmount, setLoanAmount] = useState(
    Math.round((props.fields?.MinAmount?.value || 0 + props.fields?.MaxAmount?.value || 0) / 2)
  );
  ...
  useEffect(() => {
    ...
    setTotalDebt(totalDebtCalculation || 0);
    ...
    >
      ...
      <div className="loan-calculator-results">
        ...
        <ResultLine
          ...
          right={
            <>
              {(totalDebt || 0).toFixed(2)} <Text field={props.fields.Currency} />
            </>
          }
        />
      </div>
